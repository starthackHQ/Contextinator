...
def git_root(path: Optional[str] = None) -> str:
    ...
    try:
        result = run(
            ["git"] + path_params + ["rev-parse", "--show-toplevel"],
            capture_output=True,
            text=True,
            check=True,
        )
...
def clone_repo(repo_url: str, target_dir: Optional[str] = None) -> str:
    ...
    try:
        ...
        raise FileSystemError(
            f"Failed to clone repository: {e}", path=target_dir, operation="clone"
        )


def resolve_repo_path(
    repo_url: Optional[str] = None, path: Optional[str] = None
) -> str:
    """
...
def is_valid_git_url(url: Optional[str]) -> bool:
    ...
    valid_patterns = ["github.com", "gitlab.com", "bitbucket.org", ".git"]
