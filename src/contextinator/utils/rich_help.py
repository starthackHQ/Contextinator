            ...
            for line in lines:
                ...
                if stripped in ["positional arguments:", "arguments:"]:
                    if description_lines:
                        ...
                        description_lines = []
                    rich_console.print(
                        "[bold yellow]Positional Arguments[/bold yellow]"
                    )
                    rich_console.print()
                    in_positional = True
                    in_description = False
...
[/bold bright_magenta]"""
    ...
    console.print(
        "     [cyan]contextinator chunk-embed-store-embeddings --repo-url <URL> --save[/cyan]"
    )
    console.print()
    ...
    processing_table.add_row("chunk", "Break codebase into semantic chunks")
    processing_table.add_row("embed", "Generate embeddings for chunks using OpenAI")
    processing_table.add_row(
        "store-embeddings", "Store embeddings in ChromaDB vector database"
    )
    processing_table.add_row(
    ...
    search_table.add_row("search", "Semantic search using natural language")
    search_table.add_row("symbol", "Find functions/classes by name")
    search_table.add_row("pattern", "Search for text patterns or regex")
    search_table.add_row("read-file", "Reconstruct complete file from chunks")
    search_table.add_row("search-advanced", "Advanced search with multiple filters")
    ...
    db_table.add_row("db-info", "Show database information and statistics")
    db_table.add_row("db-list", "List all collections")
    db_table.add_row("db-show", "Show details of a specific collection")
    db_table.add_row("db-clear", "Delete a specific collection")
    ...
    examples = [
        (
            ...
            "contextinator chunk-embed-store-embeddings --repo-url https://github.com/user/repo --save",
        ),
        (
            "Process local directory",
            "contextinator chunk-embed-store-embeddings --path /path/to/repo --save",
        ),
        (
            "Search for authentication code",
            'contextinator search "authentication logic" -c MyRepo -n 5',
        ),
        (
        ...
        (
            "Search with filters",
            'contextinator search-advanced -c MyRepo --semantic "error handling" --language python',
        ),
