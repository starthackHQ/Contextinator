...
CHROMA_DB_DIR: str = (
    ".contextinator/chromadb"  # Relative directory for ChromaDB storage
)
...
def validate_config() -> None:
    ...
    if CHUNK_OVERLAP >= MAX_TOKENS:
        raise ConfigurationError(
            "CHUNK_OVERLAP must be less than MAX_TOKENS", "CHUNK_OVERLAP"
        )

    if EMBEDDING_BATCH_SIZE <= 0:
        raise ConfigurationError(
            "EMBEDDING_BATCH_SIZE must be positive", "EMBEDDING_BATCH_SIZE"
        )

    if CHROMA_BATCH_SIZE <= 0:
        raise ConfigurationError(
            "CHROMA_BATCH_SIZE must be positive", "CHROMA_BATCH_SIZE"
        )
