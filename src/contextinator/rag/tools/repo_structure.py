...
def format_tree_string(node, prefix="", is_last=True):
    ...
    if "children" in node:
        ...
        for i, child in enumerate(children):
            lines.append(
                format_tree_string(child, prefix + extension, i == len(children) - 1)
            )
...
async def analyze_structure(
    ...
    tree = await loop.run_in_executor(
        None, lambda: build_tree_dict(path, ignore_patterns, max_depth)
    )
    ...
    if output_file:
        await loop.run_in_executor(
            None, lambda: Path(output_file).write_text(output, encoding="utf-8")
        )
