...
class ChromaVectorStore:
    ...
    def __init__(
        ...
        if db_path:
            ...
            self.db_path = str(
                get_storage_path(base_dir, "chromadb", repo_name, custom_chromadb_dir)
            )
        else:
        ...
        try:
            ...
            if USE_CHROMA_SERVER:
                raise VectorStoreError(
                    f"Both server and local ChromaDB initialization failed: {e}",
                    "initialize",
                )
            else:
                raise VectorStoreError(
                    f"Local ChromaDB initialization failed: {e}", "initialize"
                )
    ...
    def _initialize_local_client(self) -> None:
        ...
        try:
            ...
            raise FileSystemError(
                f"Cannot create ChromaDB directory: {e}", self.db_path, "create"
            )
        ...
        try:
            ...
            raise VectorStoreError(
                f"Failed to create local ChromaDB client: {e}", "initialize"
            )
        ...
        try:
            ...
            try:
                ...
                collection = self.client.create_collection(
                    name=safe_name,
                    metadata={
                        "description": f"Code chunks for repository: {collection_name}"
                    },
    ...
    def _prepare_batch_data(
        ...
        for i, chunk in enumerate(embedded_chunks):
            ...
            if not chunk_id:
                ...
                chunk_id = (
                    f"chunk_{i}_{chunk.get('hash', hash(chunk.get('content', '')))}"
                )
            ids.append(chunk_id)
            ...
            metadata = {
                ...
                if k not in ["embedding", "enriched_content"]
            }
    ...
    def store_embeddings(
        ...
        if not collection_name:
            raise ValidationError(
                "Collection name cannot be empty", "collection_name", "non-empty string"
            )
        ...
        for batch_idx in range(0, len(embedded_chunks), batch_size):
            ...
            try:
                ids, embeddings, metadatas, documents = self._prepare_batch_data(
                    batch_chunks
                )
        ...
        if stored_count == 0:
            raise VectorStoreError(
                "All batches failed - no embeddings stored", "store", collection_name
            )
        ...
        if failed_batches:
            logger.warning(
                f"Failed to store {len(failed_batches)} batches: {failed_batches}"
            )
        ...
        if failed_batches:
            logger.warning(
                f"⚠️  {len(failed_batches)} batches failed out of {total_batches}"
            )
    ...
    def list_collections(self) -> List[Dict[str, Any]]:
        ...
        if not self.client:
            raise VectorStoreError(
                "ChromaDB client not initialized", "list_collections"
            )
        ...
        try:
            ...
            for col in collections:
                try:
                    ...
                    logger.warning(
                        f"Error getting info for collection '{col.name}': {e}"
                    )
                    # Add partial info
                    result.append(
                        {
...
def store_repository_embeddings(
    ...
    vector_store = ChromaVectorStore(
        base_dir=base_dir, repo_name=repo_name, custom_chromadb_dir=custom_chromadb_dir
    )
...
def get_repository_collection_info(
    repo_name: str, collection_name: Optional[str] = None
) -> Dict[str, Any]:
    """
