import argparse
import sys
from contextinator.rag.utils import resolve_repo_path, logger
from contextinator.rag.utils.exceptions import FileSystemError
from contextinator.rag.utils.rich_help import print_main_help, RichHelpFormatter
import os
from contextinator.rag.chunking import chunk_repository
from contextinator.rag.config import get_storage_path


def chunk_func(args):
    from pathlib import Path
    from contextinator.rag.utils.repo_utils import extract_repo_name_from_url

    repo_url = getattr(args, "repo_url", None)

    try:
        repo_path = resolve_repo_path(
            repo_url=repo_url, path=getattr(args, "path", None)
        )
    ...
    custom_chunks_dir = getattr(
        args, "chunks_dir", None
    ...
    if args.save:
        chunks_path = get_storage_path(
            output_dir, "chunks", repo_name, custom_chunks_dir
        )
...
def embed_func(args):
    ...
    from contextinator.rag.embedding import embed_chunks
    from contextinator.rag.utils import resolve_repo_path, logger
    from contextinator.rag.utils.repo_utils import extract_repo_name_from_url
    from pathlib import Path
    import os
    ...
    try:
        ...
        logger.info(
            f"Embedding generation complete: {len(embedded_chunks)} chunks embedded"
        )

        if args.save:
            embeddings_path = get_storage_path(
                base_dir, "embeddings", repo_name, custom_embeddings_dir
            )
...
def structure_func(args):
    ...
    import asyncio
    from contextinator.rag.tools.repo_structure import analyze_structure_async
    from contextinator.rag.utils import resolve_repo_path, logger
...
def store_embeddings_func(args):
    ...
    from contextinator.rag.embedding import load_embeddings
    from contextinator.rag.vectorstore import store_repository_embeddings
    from contextinator.rag.utils import resolve_repo_path, logger
    from contextinator.rag.utils.repo_utils import extract_repo_name_from_url
    from pathlib import Path
    import os
    ...
    try:
        ...
        stats = store_repository_embeddings(
            base_dir, repo_name, embedded_chunks, collection_name, custom_chromadb_dir
        )
...
def pipeline_func(args):
    ...
    try:
        ...
        collection_name = getattr(args, "collection_name", None) or repo_name
        stats = store_repository_embeddings(
            base_dir, repo_name, embedded_chunks, collection_name
        )
        ...
        if args.save:
            chunks_path = get_storage_path(
                base_dir, "chunks", repo_name, custom_chunks_dir
            )
            embeddings_path = get_storage_path(
                base_dir, "embeddings", repo_name, custom_embeddings_dir
            )
            logger.info(
                "   💾 Artifacts saved in: %s and %s", chunks_path, embeddings_path
            )
...
def async_batch_func(args):
    ...
    import asyncio
    import json
    from contextinator.rag.ingestion import AsyncIngestionService
    from pathlib import Path
    ...
    try:
        ...
        for i, repo in enumerate(repos):
            if "repo_url" not in repo or "collection_name" not in repo:
                logger.error(
                    f"Repo {i} missing required fields: repo_url, collection_name"
                )
                sys.exit(1)
        ...
        async def run_batch():
            ...
            results = await service.process_batch_async(
                repos=repos, max_concurrent=max_concurrent, cleanup=cleanup
            )
            return results
        ...
        if failed_count > 0:
            ...
            for r in results:
                if r.get("status") == "failed":
                    logger.info(
                        f"   - {r.get('repo_url', 'unknown')}: {r.get('error', 'unknown error')}"
                    )
...
def search_func(args):
    ...
    from contextinator.rag.tools import semantic_search
    import asyncio
    from contextinator.rag.utils.output_formatter import (
...
def symbol_func(args):
    ...
    from contextinator.rag.tools import symbol_search
    import asyncio
    from contextinator.rag.utils.output_formatter import (
        format_search_results,
        export_results_json,
    ...
    try:
        ...
        if not args.json and not getattr(args, "toon", None):
            format_search_results(
                results, query=f"Symbol: {args.symbol_name}", collection=args.collection
            )
...
def cat_file_func(args):
    ...
    import asyncio
    from contextinator.rag.tools import cat_file
    from contextinator.rag.utils.output_formatter import export_results_json
    ...
    try:
        ...
        if args.json:
            export_results_json(
                {"file_path": args.file_path, "content": content}, args.json
            )
        else:
...
def grep_func(args):
    ...
    from contextinator.rag.tools import grep_search
    import asyncio
    from contextinator.rag.utils.output_formatter import export_results_json
    ...
    try:
        ...
        if args.json:
            ...
            print(
                f"Found {results['total_matches']} matches in {results['total_files']} files\n"
            )
...
def read_file_func(args):
    ...
    import asyncio
    from contextinator.rag.tools import cat_file
    from contextinator.rag.utils.output_formatter import export_results_json, export_results_toon
    ...
    try:
        ...
        file_data = {"file_path": args.file_path, "content": content}
...
def search_advanced_func(args):
    ...
    from contextinator.rag.tools import semantic_search, grep_search
    from contextinator.rag.utils.output_formatter import (
        format_search_results,
        export_results_json,
        export_results_toon,
    ...
    try:
        ...
        if args.semantic:
            ...
            if args.json:
                ...
                format_search_results(
                    results, query=query_desc, collection=args.collection
                )
...
def db_info_func(args):
    ...
    from contextinator.rag.vectorstore import ChromaVectorStore
    import os
    from pathlib import Path
    ...
    try:
        ...
        vector_store = ChromaVectorStore(
            base_dir=base_dir,
            repo_name=repo_name,
            custom_chromadb_dir=custom_chromadb_dir,
        )
        ...
        for collection in collections:
            ...
            if collection.get("metadata"):
                logger.info(
                    "     📝 Description: %s",
                    collection["metadata"].get("description", "N/A"),
                )
            logger.info("")
...
def db_list_func(args):
    ...
    try:
        ...
        for collection in collections:
            logger.info(
                "  - %s (%d documents)", collection["name"], collection["count"]
            )
...
def db_show_func(args):
    ...
    from contextinator.rag.vectorstore import ChromaVectorStore
    from contextinator.rag.config import sanitize_collection_name
    import os
    from pathlib import Path
...
def db_clear_func(args):
    ...
    try:
        ...
        if not args.force:
            response = input(
                f"⚠️  Are you sure you want to delete collection '{collection_name}'? (y/N): "
            )
            if response.lower() != "y":
...
def main():
    ...
    p_chunk = sub.add_parser(
        "chunk",
        help="Chunks the local Git codebase into semantic units and (optionally) save them",
        formatter_class=RichHelpFormatter,
    )
    p_chunk.add_argument(
        "--save",
        action="store_true",
        help="Save chunks to .contextinator/chunks/ folder",
    )
    p_chunk.add_argument(
        "--save-ast",
        action="store_true",
        help="Save AST trees for analysis and debugging",
    )
    p_chunk.add_argument(
        "--repo-url", help="GitHub/Git repository URL to clone and chunk"
    )
    p_chunk.add_argument(
        "--path", help="Local path to repository (default: current directory)"
    )
    p_chunk.add_argument(
        "--output",
        "-o",
        help="Output directory for chunks (default: current directory)",
    )
    p_chunk.add_argument(
        "--chunks-dir",
        help="Custom chunks directory (overrides default .contextinator/chunks)",
    )
    ...
    p_embed = sub.add_parser(
        "embed",
        help="Generate embeddings for existing chunks using OpenAI and (optionally) save them",
        formatter_class=RichHelpFormatter,
    )
    p_embed.add_argument(
        "--save",
        action="store_true",
        help="Save embeddings to .contextinator/embeddings/ folder",
    )
    p_embed.add_argument(
        "--repo-url", help="GitHub/Git repository URL to clone and embed"
    )
    p_embed.add_argument(
        "--path", help="Local path to repository (default: current directory)"
    )
    p_embed.add_argument(
        "--output",
        "-o",
        help="Base directory containing chunks folder (default: current directory)",
    )
    p_embed.add_argument(
        "--chunks-dir",
        help="Custom chunks directory (overrides default .contextinator/chunks)",
    ...
    p_embed.add_argument(
        "--embeddings-dir",
        help="Custom embeddings directory (overrides default .contextinator/embeddings)",
    )
    p_embed.add_argument(
        "--api-key", help="OpenAI API key (alternative to OPENAI_API_KEY env var)"
    )
    ...
    p_store = sub.add_parser(
        "store-embeddings",
        help="Load embeddings into ChromaDB vector store",
        formatter_class=RichHelpFormatter,
    )
    ...
    p_store.add_argument(
        "--output",
        "-o",
        help="Base directory containing embeddings folder (default: current directory)",
    )
    p_store.add_argument(
        "--embeddings-dir",
        help="Custom embeddings directory (overrides default .contextinator/embeddings)",
    ...
    p_store.add_argument(
        "--chromadb-dir",
        help="Custom chromadb directory (overrides default .contextinator/chromadb)",
    )
    p_store.add_argument(
        "--repo-name",
        help="Repository name (for locating embeddings when not using --repo-url or --path)",
    )
    p_store.add_argument(
        "--collection-name", help="Custom collection name (default: repository name)"
    )
    ...
    p_pipeline = sub.add_parser(
        ...
        help="Run chunk, embed and store-embeddings in a single command",
        formatter_class=RichHelpFormatter,
    )
    p_pipeline.add_argument(
        "--save",
        action="store_true",
        help="Save intermediate artifacts (chunks + embeddings)",
    )
    p_pipeline.add_argument(
        "--repo-url", help="GitHub/Git repository URL to clone and process"
    )
    p_pipeline.add_argument(
        "--path", help="Local path to repository (default: current directory)"
    )
    p_pipeline.add_argument(
        "--output", "-o", help="Base output directory (default: current directory)"
    )
    p_pipeline.add_argument(
        "--chunks-dir",
        help="Custom chunks directory (overrides default .contextinator/chunks)",
    ...
    p_query = sub.add_parser(
        "query",
        help="Query the vector store for semantically similar code chunks",
        formatter_class=RichHelpFormatter,
    )
    p_query.add_argument(
        "query_text",
        nargs=argparse.REMAINDER,
        help="Query text (wrap in quotes if multiple words)",
    )
    p_query.add_argument(
        "--n-results", "-n", type=int, default=5, help="Number of results to return"
    )
    p_query.add_argument(
        "--save", metavar="FILE", help="Save results to a file (JSON or text)"
    )
    p_query.set_defaults(func=query_func)

    # db-info
    p_db_info = sub.add_parser(
        "db-info",
        help="Show ChromaDB database information and statistics",
        formatter_class=RichHelpFormatter,
    )
    p_db_info.add_argument(
        "--repo-name",
        help="Repository name (for locating database when not using --output)",
    )
    p_db_info.add_argument(
        "--chromadb-dir",
        help="Custom chromadb directory (overrides default .contextinator/chromadb)",
    ...
    # db-list
    p_db_list = sub.add_parser(
        "db-list",
        help="List all collections in ChromaDB",
        formatter_class=RichHelpFormatter,
    )
    ...
    # db-show
    p_db_show = sub.add_parser(
        "db-show",
        help="Show details of a specific collection",
        formatter_class=RichHelpFormatter,
    )
    p_db_show.add_argument("collection_name", help="Name of the collection to show")
    p_db_show.add_argument(
        "--sample", type=int, default=0, help="Show sample documents (specify number)"
    )
    p_db_show.add_argument(
        "--repo-name",
        help="Repository name (for locating database when not using --output)",
    ...
    p_structure = sub.add_parser(
        "structure",
        help="Analyze and display repository structure as a tree",
        formatter_class=RichHelpFormatter,
    )
    ...
    p_structure.add_argument(
        "--depth", type=int, help="Maximum depth to traverse (default: unlimited)"
    )
    p_structure.add_argument(
        "--format",
        choices=["tree", "json"],
    ...
    p_search.add_argument(
        "--n-results", "-n", type=int, default=5, help="Number of results (default: 5)"
    )
    ...
    p_search.add_argument(
        "--include-parents",
        action="store_true",
        help="Include parent chunks (classes/modules) in results",
    )
    ...
    p_cat = sub.add_parser(
        "cat",
        help="Display complete file contents from chunks",
        formatter_class=RichHelpFormatter,
    )
    ...
    p_grep = sub.add_parser(
        "grep",
        help="Advanced grep search with optional regex support",
        formatter_class=RichHelpFormatter,
    )
    ...
    p_grep.add_argument(
        "--limit", type=int, default=100, help="Maximum chunks to search (default: 100)"
    )
    p_grep.add_argument(
        "--regex", action="store_true", help="Enable regex pattern matching"
    )
    p_grep.add_argument(
        "--case-sensitive", action="store_true", help="Case-sensitive matching"
    )
    p_grep.add_argument(
        "--whole-word", action="store_true", help="Match whole words only"
    )
    p_grep.add_argument(
        "--context",
        type=int,
    ...
    p_read_file = sub.add_parser(
        "read-file",
        help="Reconstruct and display complete file from chunks",
        formatter_class=RichHelpFormatter,
    )
    ...
    p_read_file.add_argument(
        "--no-join", action="store_true", help="Show chunks separately (don't join)"
    )
    ...
    p_search_adv.add_argument(
        "--collection", "-c", required=True, help="Collection name"
    )
    p_search_adv.add_argument(
        "--semantic", "-s", help="Semantic query for hybrid search"
    )
    ...
    p_search_adv.add_argument(
        "--file", "-f", help="Filter by file path (partial match)"
    )
    ...
    p_search_adv.add_argument(
        "--toon", help="Export results to TOON file (compact format)"
    )
    p_search_adv.add_argument(
        "--chromadb-dir",
        help="Custom chromadb directory (overrides default .contextinator/chromadb)",
    ...
    p_async_batch.add_argument(
        "repos_file", help="JSON file containing list of repositories"
    )
    p_async_batch.add_argument(
        "--output", "-o", help="Base output directory (default: current directory)"
    )
    ...
    p_async_batch.add_argument(
        "--no-cleanup", action="store_true", help="Keep temporary cloned directories"
    )
    p_async_batch.add_argument(
        "--results", dest="results_file", help="Save results to JSON file"
    )
