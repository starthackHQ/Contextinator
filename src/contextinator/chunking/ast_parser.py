...
try:
    ...
    logger.info(
        "💡 Install missing modules with: pip install tree-sitter tree-sitter-python tree-sitter-javascript ..."
    )
...
def parse_file(
    ...
    if save_ast and chunks_dir is None:
        raise ValidationError(
            "chunks_dir is required when save_ast=True", "chunks_dir", "Path object"
        )
    ...
    try:
        ...
        try:
            ...
            if not parser:
                logger.warning(
                    f"No parser available for {language}, using fallback for {file_path}"
                )
        ...
        if not nodes:
            logger.warning(
                f"No semantic nodes found in {file_path}, using file-level chunking"
            )
            result = _fallback_parse(file_path, file_path_str, language, content)
        else:
            result = {
...
def _fallback_parse(
    file_path: Path, file_path_str: str, language: str, content: str
) -> Dict[str, Any]:
    """
...
def extract_nodes(root_node: Any, content: str, language: str) -> List[Dict[str, Any]]:
    ...
    def traverse(
        node: Any, parent_id: Optional[str] = None, parent_info: Optional[Dict] = None
    ) -> None:
        """Recursively traverse AST and extract target nodes with hierarchy tracking."""
        if node.type in target_types:
            node_id = str(uuid.uuid4())
            node_content = content_bytes[node.start_byte : node.end_byte].decode(
                "utf-8", errors="ignore"
            )
    ...
    for node in nodes:
        if node["is_parent"]:
            node["children_ids"] = [
                n["id"] for n in nodes if n["parent_id"] == node["id"]
            ]
...
def get_node_name(node: Any, content_bytes: bytes) -> Optional[str]:
    ...
    try:
        ...
        return (
            f"unknown_line_{node.start_point[0] + 1}"
            if hasattr(node, "start_point")
            else None
        )


def _save_ast_safely(
    file_path: Path,
    language: str,
    root_node: Any,
    content: str,
    ...
    chunks_dir: Path,
    tree_info: Optional[Dict[str, Any]],
) -> None:
    """
    ...
    try:
        ...
        save_ast_visualization(
            str(file_path), language, root_node, content, nodes, chunks_dir, tree_info
        )
    except Exception as e:
